function [s, b, l, idx] = event(s_in, b_in)
%#codegen

minus_1 = zeros(1, 65000);
...
for idx = 1:numel(b_in),
    ...
    if (and(b_in(idx) > 0, sum(b_in(row+1:size(b_in, 2), col)) == 0))%если выше нет лат. связей
        if (col == 13)%13 пф
            ...
            if (and(s_in(idx) == 1, s_in(row+1,1) == 1))%если оба GDP
                k_break = 800;
            ...
            if (col == 1)
                if (and(and(b_in(row-2, 13) == 2, b_in(row, 2) == 1), b_in(row-1, 13) >= 1))
                    pi = 1000;
                else
                    pi = 1;
                end
            else
                if(and(b_in(row, col-1) == 1, b_in(row, col+1) == 1))
                    pi = 1000;
                else
                    pi = 1;
                end
...
final_list_1 = [minus_1, plus_1, break_lateral_1, bond_lateral_1];
...
if (final_list_1(least_time_1) == 1)
    s_in(idx) = 0;
